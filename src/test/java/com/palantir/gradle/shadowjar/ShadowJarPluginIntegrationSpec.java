...
class ShadowJarPluginIntegrationSpec {
    ...
    @Test
    void should_only_include_shadeTransitively_dependencies_in_jar_not_implementation_or_runtimeOnly_dependencies(
            GradleInvoker gradle, RootProject project) {
        project.buildGradle().plugins().add("java-library");
        project.buildGradle().append("""
            dependencies {
        ...
        assertThat(dependenciesText).contains("<artifactId>checker-qual</artifactId>");
        assertThat(dependenciesText).contains("<artifactId>apiguardian-api</artifactId>");
        assertThat(dependenciesText).contains("<artifactId>jsr305</artifactId>");
        ...
        assertThat(jarEntryNames).contains(relocatedClass("com/google/common/collect/ImmutableList.class"));

        // Implementation/runtimeOnly/api dependencies should NOT be in jar at all (neither shaded nor unshaded)
        assertThat(jarEntryNames).doesNotContain("org/checkerframework/checker/nullness/qual/Nullable.class");
        assertThat(jarEntryNames)
                .doesNotContain(relocatedClass("org/checkerframework/checker/nullness/qual/Nullable.class"));
        assertThat(jarEntryNames).doesNotContain("org/apiguardian/api/API.class");
        assertThat(jarEntryNames).doesNotContain(relocatedClass("org/apiguardian/api/API.class"));
